...
parallel -j 36 'kraken2-build --add-to-library seq_gtdb_itegrt_v1/{}_pseudoTaxID.fa --db ../bacteria_gtdb_itegrt_v1' \
...
cut -d ' ' -f 1,12,15,26,29,40,43 inner_join_extr_cf_std_mag_itergrt  | sed -e 's/ /\t/g' -e 's/(//g' -e 's/%)//g' >mag_3group_report.tsv
...
awk -F ' ' '{print $2,$1,$3,$4,$5,$6,$7,$8}' mag_custom_3group.tsv > a_final_kraken_mag_custom_3group.tsv

cp a_final_kraken_mag_itegr_custom_3group_v1.tsv result/
